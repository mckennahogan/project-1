...
diff.means<-function(data0,data1,data2,R){
  ...
  # nodibindic <- rep("No Diabetes - Prediabetes",1000)
  # predibindic <- rep("Prediabetes - Diabetes",1000)
  # dibindic <- rep("Diabetes - No Diabetes",1000)
  # indic <- c(nodibindic,predibindic,dibindic)
  # 
  # combination<-c(no_pre$t,pre_diab$t,diab_no$t)
  # hist.stuff<-data.frame(combination,group=indic)
  # overall.hist<- ggplot(data=hist.stuff,aes(x=combination,fill=group))+
  #   geom_histogram()+    
  #   geom_segment(aes(y=400,yend=400,x=boot.no_pre$percent[4],
  #                    xend=boot.no_pre$percent[5], col="percentile"),size=1.5)+
  #   geom_segment(aes(y=420,yend=420,x=boot.no_pre$basic[4],
  #                    xend=boot.no_pre$basic[5],col="basic"),size=1.5)+
  #   geom_histogram(aes(pre_diab$t),fill="deepskyblue") +
  #   geom_segment(aes(y=400,yend=400,x=boot.pre_diab$percent[4],
  #                    xend=boot.pre_diab$percent[5], col="percentile"),size=1.5)+
  #   geom_segment(aes(y=420,yend=420,x=boot.pre_diab$basic[4],
  #                    xend=boot.pre_diab$basic[5],col="basic"),size=1.5)+
  #   geom_segment(aes(y=400,yend=400,x=boot.diab_no$percent[4],
  #                    xend=boot.diab_no$percent[5], col="percentile"),size=1.5)+
  #   geom_segment(aes(y=420,yend=420,x=boot.diab_no$basic[4],
  #                    xend=boot.diab_no$basic[5],col="basic"),size=1.5)+
  #   xlab("groups")
...
```{r echo=FALSE, message=FALSE, warning=FALSE, out.width='30%', out.height='30%'}
HighBP.list[[1]]
HighBP.list[[3]]
HighBP.list[[5]]
```
...
As you can see from these histograms, 0 is almsot never included in the interval. The only times 0 is in the interval is for AnyHealthcare. This means that the difference in means between no no diabetes and prediabetes, prediabetes and diabetes, and diabetes and no diabetes are significant for every variable but AnyHealthcare. The fact that the basic and percentile CI's are extremely similar show that there is no bias in the samples. This matched the outputted values for SE.
