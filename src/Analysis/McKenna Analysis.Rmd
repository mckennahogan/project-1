...
percentile_basic_bootstrap<-function(data,m,n){
  ...
  mean.kable<-kable(mean.table,align="c",captioin="means")
  
  #percentile CI's
  p.CI.HighBP<-quantile(bootstrap.reps$HighBP,c(.025,.975))
  p.CI.HighChol<-quantile(bootstrap.reps$HighChol,c(.025,.975))
  p.CI.BMI<-quantile(bootstrap.reps$BMI,c(.025,.975))
  
  p.CI.BMI.test<-c(mean.BMI-1.96*sqrt((mean.BMI*(1-mean.BMI))/m),
                   mean.BMI+1.96*sqrt((mean.BMI*(1-mean.BMI))/m))
  
  p.CI.AnyHealthcare<-quantile(bootstrap.reps$AnyHealthcare,c(.025,.975))
  ...
  p.CI.table<-data.frame(p.CI.HighBP,p.CI.HighChol,
                         p.CI.BMI,p.CI.BMI.test,p.CI.AnyHealthcare,p.CI.GenHlth,p.CI.Income)
  colnames(p.CI.table)<-c("High BP","High Chol",
                        "BMI","test","Any Healthcare","General Health","Income")
  p.CI.kable<-kable(p.CI.table,align="c",caption="percentile CI")
  ...
  colnames(b.CI.table)<-c("High BP","High Chol",
                        "BMI","Any Healthcare","General Health","Income")
  b.CI.kable<-kable(b.CI.table,align="c",caption="basic CI")
