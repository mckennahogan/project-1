...
diff.means.percentile<-function(data0,data1,data2,R){
  ...
  plot.pre_diab<-ggplot() +
    geom_histogram(aes(pre_diab$t)) +
    xlab("mean(pre diabetes) - mean(diabetes)")
  plot.diab_no<-ggplot() +
    geom_histogram(aes(diab_no$t)) +
  ...
  boot.no_pre<-boot.ci(no_pre,type="percentile")
  boot.pre_diab<-boot.ci(pre_diab,type="percentile")
  boot.diab_no<-boot.ci(diab_no.boot,type="percentile")
...
diff.means.percentile(HighBP0,HighBP1,HighBP2,500)
```
